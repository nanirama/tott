...
const Footer = () => {
   ...
   const footerData = data && data.footer && data.footer.data && data.footer.data.attributes ? data.footer.data.attributes : ''
            ...
            <div className="max-w-7xl mx-auto px-4">
               <div className="flex lg:flex-row flex-col border-t border-b py-10">
                  <div className="w-full lg:w-4/12">
                     {footerData.logo && footerData.logo.data && footerData.logo.data.attributes && footerData.logo.data.attributes.url && (
                        <Link href="/">
                           <a> <ImgLoader src={footerData.logo.data.attributes.url} width={148} height={50} /></a></Link>
                     )}

                     <p className="md:mt-8 mt-4 text-gray-500 text-base lg:mb-0 mb-12">{footerData.description && footerData.description}</p>
                  </div>
                  <div className="w-full lg:w-8/12">
                     <div className="flex flex-row md:flex-nowrap flex-wrap md:justify-around justify-start">
                        {footerData.nav_items && footerData.nav_items.map((item, index) => {
                           return (
                              <div className="flex flex-col w-6/12 md:mb-0 mb-5" key={index}>
                                 <h5 className="text-gray-400 text-sm font-semibold mb-4">{item.heading && item.heading}</h5>
                                 <ul>
                                    {item.items && item.items.length > 0 && item.items.map((navItem, navIndex) => {
                                       if (checkLinkHandler(navItem.button_url)) {
                                          return (
                                             <li key={navIndex}>
                                             <a className="text-gray-500 text-base font-medium mb-3 inline-block" rel="noopener noreferrer" target={'_blank'} href={navItem.button_url}>
                                                {navItem.button_text}
                                             </a>
                                             </li>
                                          )
                                          ...
                                          return (
                                             <li key={navIndex}>
                                                <Link href={`/${navItem.button_url}`} >
                                                   <a className="text-gray-500 text-base font-medium mb-3 inline-block">{navItem.button_text}</a>
                                                </Link>
                                             </li>
                                          )
                                       }
               ...
               <div className="flex md:flex-row flex-col-reverse">
                  <div className="w-full md:w-6/12">
                     <p className="text-base text-gray-400 mb-0">
                        {footerData.copyright_disclaimer && (
                           <ReactMarkdown>{footerData.copyright_disclaimer}</ReactMarkdown>
                        )}
                  ...
                  <div className="w-full md:w-6/12">
                     <div className="s_links flex md:justify-end justify-start md:mb-0 mb-5">
                        ...
                        {footerData.social_links && footerData.social_links.platform && footerData.social_links.platform.map((item, index) => {
                           if (item.button_text === 'Twitter') {
                              return (
                                 <a href={`${item.button_url}`} target={'_blank'} key={index} rel="noopener noreferrer">
