import React from 'react';
export default function Lovetohearsection() {
    return (
        <div className="max-w-7xl mx-auto px-4 lg:mt-20 mt-16">
            <div className='w-full text-base font-semibold'>Contact us</div>
            <h2 className='w-full font-semibold text-4xl pt-3'>We’d love to hear from you</h2>
            <p className='text-xl font-normal pt-5'>Our friendly team is always here to chat.</p>
            <div className='w-100 my-16 grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-6'>
                <div className='w-full bg-gray-50 p-6'>
                    <div className='p-3 bg-gray-500 rounded-lg w-12 h-12'>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M22 6C22 4.9 21.1 4 20 4H4C2.9 4 2 4.9 2 6M22 6V18C22 19.1 21.1 20 20 20H4C2.9 20 2 19.1 2 18V6M22 6L12 13L2 6" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                    </div>
                    <div className='text-xl font-medium text-gray-900 mt-12'>Chat to sales</div>
                    <div className='text-base font-normal mt-2 text-gray-500 pb-6'>Speak to our friendly team.</div>
                    <a href='#' className='text-base font-normal'>sales@untitledui.com</a>
                </div>
                <div className='w-full bg-gray-50 p-6'>
                    <div className='p-3 bg-gray-500 rounded-lg w-12 h-12'>
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M19 9.50003C19.0034 10.8199 18.6951 12.1219 18.1 13.3C17.3944 14.7118 16.3098 15.8992 14.9674 16.7293C13.6251 17.5594 12.0782 17.9994 10.5 18C9.18013 18.0035 7.87812 17.6951 6.7 17.1L1 19L2.9 13.3C2.30493 12.1219 1.99656 10.8199 2 9.50003C2.00061 7.92179 2.44061 6.37488 3.27072 5.03258C4.10083 3.69028 5.28825 2.6056 6.7 1.90003C7.87812 1.30496 9.18013 0.996587 10.5 1.00003H11C13.0843 1.11502 15.053 1.99479 16.5291 3.47089C18.0052 4.94699 18.885 6.91568 19 9.00003V9.50003Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                    </div>
                    <div className='text-xl font-medium text-gray-900 mt-12'>Chat to support</div>
                    <div className='text-base font-normal mt-2 text-gray-500 pb-6'>We’re here to help.</div>
                    <a href='#' className='text-base font-normal'>support@untitledui.com</a>
                </div>
                <div className='w-full bg-gray-50 p-6'>
                    <div className='p-3 bg-gray-500 rounded-lg w-12 h-12'>
                        <svg width="20" height="24" viewBox="0 0 20 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M19 10C19 17 10 23 10 23C10 23 1 17 1 10C1 7.61305 1.94821 5.32387 3.63604 3.63604C5.32387 1.94821 7.61305 1 10 1C12.3869 1 14.6761 1.94821 16.364 3.63604C18.0518 5.32387 19 7.61305 19 10Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            <path d="M10 13C11.6569 13 13 11.6569 13 10C13 8.34315 11.6569 7 10 7C8.34315 7 7 8.34315 7 10C7 11.6569 8.34315 13 10 13Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                    </div>
                    <div className='text-xl font-medium text-gray-900 mt-12'>Visit us</div>
                    <div className='text-base font-normal mt-2 text-gray-500 pb-6'>Visit our office HQ.</div>
                    <div className='text-base font-normal text-gray-700'>100 Smith Street <br />Collingwood VIC 3066 AU</div>
                </div>
                <div className='w-full bg-gray-50 p-6'>
                    <div className='p-3 bg-gray-500 rounded-lg w-12 h-12'>
                        <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M21.0004 15.9201V18.9201C21.0016 19.1986 20.9445 19.4743 20.8329 19.7294C20.7214 19.9846 20.5577 20.2137 20.3525 20.402C20.1473 20.5902 19.905 20.7336 19.6412 20.8228C19.3773 20.912 19.0978 20.9452 18.8204 20.9201C15.7433 20.5857 12.7874 19.5342 10.1904 17.8501C7.77425 16.3148 5.72576 14.2663 4.19042 11.8501C2.5004 9.2413 1.44866 6.27109 1.12042 3.1801C1.09543 2.90356 1.1283 2.62486 1.21692 2.36172C1.30555 2.09859 1.44799 1.85679 1.63519 1.65172C1.82238 1.44665 2.05023 1.28281 2.30421 1.17062C2.5582 1.05843 2.83276 1.00036 3.11042 1.0001H6.11042C6.59573 0.995321 7.06621 1.16718 7.43418 1.48363C7.80215 1.80008 8.0425 2.23954 8.11042 2.7201C8.23704 3.68016 8.47187 4.62282 8.81042 5.5301C8.94497 5.88802 8.97408 6.27701 8.89433 6.65098C8.81457 7.02494 8.62928 7.36821 8.36042 7.6401L7.09042 8.9101C8.51398 11.4136 10.5869 13.4865 13.0904 14.9101L14.3604 13.6401C14.6323 13.3712 14.9756 13.1859 15.3495 13.1062C15.7235 13.0264 16.1125 13.0556 16.4704 13.1901C17.3777 13.5286 18.3204 13.7635 19.2804 13.8901C19.7662 13.9586 20.2098 14.2033 20.527 14.5776C20.8441 14.9519 21.0126 15.4297 21.0004 15.9201Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                    </div>
                    <div className='text-xl font-medium text-gray-900 mt-12'>Call us</div>
                    <div className='text-base font-normal mt-2 text-gray-500 pb-6'>Mon-Fri from 8am to 5pm.</div>
                    <div className='text-base font-normal'>+1 (555) 000-0000</div>
                </div>
            </div>
        </div>
    )
}