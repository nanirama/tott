...
const buttonClasses = {
  active:
    "relative w-1/2 bg-white border border-slate-300 mr-4 rounded-md py-2 text-base font-medium text-gray-900 whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:z-10 sm:w-auto sm:px-8",
  inActive:
    "ml-0.5 relative w-1/2 text-white rounded-md py-2 text-base font-medium whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:z-10 sm:w-auto sm:px-8",
};
      ...
      <div className="bg-neutral-800">
        <div className="max-w-7xl mx-auto md:pt-24 pt-10 md:pb-52 pb-44 px-4 sm:px-6 lg:px-8">
          <div className="sm:flex sm:flex-col sm:align-center">
            <div className="text-base capitalize text-neutral-200 text-center pb-2.5">
              pricing
            </div>
            {data?.title && (
        ...
        {data?.plans && (
          <div className="mt-12 space-y-4 sm:mt-16 sm:space-y-0  sm:grid sm:grid-cols-2 sm:gap-6 md:max-w-4xl md:mx-auto md:max-w-none xl:mx-0 md:grid-cols-3">
            {data?.plans?.map((plan, index) => (
              <div
                key={index}
                className="bg-white border border-gray-200 shadow-lg rounded-lg lg:p-8 px-4 py-8 divide-y divide-gray-200"
              >
                <div className="pb-6 flex flex-col">
                  <div className="text-sm px-1 bg-slate-100 rounded-full flex flex-row text-center mx-auto py-1">
                    <span className="text-sm text-gray-900 bg-white py-1 px-4 rounded-full">
                      {plan.name}
                    </span>
                    <span className="text-sm text-gray-900 px-3 py-1">
                      Most popular
                    </span>
                  </div>
                ...
                <div className="pt-2">
                  ...
                  {plan?.action_buttons &&
                    plan?.action_buttons?.map((button, index) => (
                      <Link href={button.button_url} key={index}>
                        <a className="mt-8 block w-full bg-gray-500 border bg-slate-600 rounded-md mt-4 py-3 text-sm font-semibold text-white text-center hover:bg-gray-900">
                          {button.button_text}
                        </a>
                      </Link>
                    ))}
