import ImgLoader from "../Image";

export default function Features5({ data }) {
  return (
    <div className="w-full lg:py-10 py-5">
      <div className="lg:max-w-5xl mx-auto w-full mf:mb-14 mb-10 text-center px-4">
        <h6 className="text-base font-semibold mb-3">Features</h6>
        {data?.title && (
          <h2 className="lg:text-4xl text-2xl font-semibold tracking-tight mb-4">
            {data?.title}
          </h2>
        )}

        {data?.description && (
          <p className="lg:text-xl text-base text-gray-500 lg:leading-8">
            {data?.description}
          </p>
        )}
      </div>
      <div className="max-w-7xl mx-auto flex flex-col px-4">
        <div className="grid md:grid-cols-3 grid-cols-1 lg:gap-14 gap-4">
          <div>
            {data?.features &&
              data?.features?.slice(0, 2)?.map((feature, index) => (
                <div className="w-full flex flex-col gap-3 mb-8" key={index}>
                  <svg
                    width="56"
                    height="56"
                    viewBox="0 0 56 56"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M4 28C4 14.7452 14.7452 4 28 4C41.2548 4 52 14.7452 52 28C52 41.2548 41.2548 52 28 52C14.7452 52 4 41.2548 4 28Z"
                      fill="#D9D8D8"
                    />
                    <path
                      d="M38 22C38 20.9 37.1 20 36 20H20C18.9 20 18 20.9 18 22M38 22V34C38 35.1 37.1 36 36 36H20C18.9 36 18 35.1 18 34V22M38 22L28 29L18 22"
                      stroke="#504F4F"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    />
                    <path
                      d="M28 48C16.9543 48 8 39.0457 8 28H0C0 43.464 12.536 56 28 56V48ZM48 28C48 39.0457 39.0457 48 28 48V56C43.464 56 56 43.464 56 28H48ZM28 8C39.0457 8 48 16.9543 48 28H56C56 12.536 43.464 0 28 0V8ZM28 0C12.536 0 0 12.536 0 28H8C8 16.9543 16.9543 8 28 8V0Z"
                      fill="#F1F0F0"
                    />
                  </svg>
                  <h3 className="text-xl font-medium text-gray-900">
                    {feature.title}
                  </h3>
                  <p className="text-base text-gray-500">
                    {feature.description}
                  </p>
                </div>
              ))}
            {/* <div className="w-full flex flex-col gap-3 mb-8">
              <svg
                width="56"
                height="56"
                viewBox="0 0 56 56"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M4 28C4 14.7452 14.7452 4 28 4C41.2548 4 52 14.7452 52 28C52 41.2548 41.2548 52 28 52C14.7452 52 4 41.2548 4 28Z"
                  fill="#D9D8D8"
                />
                <path
                  d="M29 18L19 30H28L27 38L37 26H28L29 18Z"
                  stroke="#504F4F"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M28 48C16.9543 48 8 39.0457 8 28H0C0 43.464 12.536 56 28 56V48ZM48 28C48 39.0457 39.0457 48 28 48V56C43.464 56 56 43.464 56 28H48ZM28 8C39.0457 8 48 16.9543 48 28H56C56 12.536 43.464 0 28 0V8ZM28 0C12.536 0 0 12.536 0 28H8C8 16.9543 16.9543 8 28 8V0Z"
                  fill="#F1F0F0"
                />
              </svg>

              <h3 className="text-xl font-medium text-gray-900">
                Deliver instant answers
              </h3>
              <p className="text-base text-gray-500">
                An all-in-one customer service platform that helps you balance
                everything your customers need to be happy.
              </p>
            </div> */}
          </div>

          {data?.image && (
            <div className="">
              <ImgLoader
                src={data?.image?.data?.attributes?.url}
                width={552}
                height={516}
                alt={data?.image?.data?.attributes?.alternativeText}
              />
            </div>
          )}

          <div>
            {data?.features &&
              data?.features
                ?.slice(2, data?.features.length)
                ?.map((feature, index) => (
                  <div className="w-full flex flex-col gap-3 mb-8" key={index}>
                    <svg
                      width="56"
                      height="56"
                      viewBox="0 0 56 56"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M4 28C4 14.7452 14.7452 4 28 4C41.2548 4 52 14.7452 52 28C52 41.2548 41.2548 52 28 52C14.7452 52 4 41.2548 4 28Z"
                        fill="#D9D8D8"
                      />
                      <path
                        d="M34 36V26M28 36V20M22 36V30"
                        stroke="#504F4F"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                      />
                      <path
                        d="M28 48C16.9543 48 8 39.0457 8 28H0C0 43.464 12.536 56 28 56V48ZM48 28C48 39.0457 39.0457 48 28 48V56C43.464 56 56 43.464 56 28H48ZM28 8C39.0457 8 48 16.9543 48 28H56C56 12.536 43.464 0 28 0V8ZM28 0C12.536 0 0 12.536 0 28H8C8 16.9543 16.9543 8 28 8V0Z"
                        fill="#F1F0F0"
                      />
                    </svg>

                    <h3 className="text-xl font-medium text-gray-900">
                      {feature.title}
                    </h3>
                    <p className="text-base text-gray-500">
                      {feature.description}
                    </p>
                  </div>
                ))}
            {/* <div className="w-full flex flex-col gap-3 mb-8">
              <svg
                width="56"
                height="56"
                viewBox="0 0 56 56"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M4 28C4 14.7452 14.7452 4 28 4C41.2548 4 52 14.7452 52 28C52 41.2548 41.2548 52 28 52C14.7452 52 4 41.2548 4 28Z"
                  fill="#D9D8D8"
                />
                <path
                  d="M24 30C24 30 25.5 32 28 32C30.5 32 32 30 32 30M25 25H25.01M31 25H31.01M38 28C38 33.5228 33.5228 38 28 38C22.4772 38 18 33.5228 18 28C18 22.4772 22.4772 18 28 18C33.5228 18 38 22.4772 38 28Z"
                  stroke="#504F4F"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M28 48C16.9543 48 8 39.0457 8 28H0C0 43.464 12.536 56 28 56V48ZM48 28C48 39.0457 39.0457 48 28 48V56C43.464 56 56 43.464 56 28H48ZM28 8C39.0457 8 48 16.9543 48 28H56C56 12.536 43.464 0 28 0V8ZM28 0C12.536 0 0 12.536 0 28H8C8 16.9543 16.9543 8 28 8V0Z"
                  fill="#F1F0F0"
                />
              </svg>

              <h3 className="text-xl font-medium text-gray-900">
                Connect with customers
              </h3>
              <p className="text-base text-gray-500">
                Solve a problem or close a sale in real-time with chat. If no
                one is available, customers are seamlessly routed to email
                without confusion.
              </p>
            </div> */}
          </div>
        </div>
      </div>
    </div>
  );
}
