...
export default function Team({ profiles }) {
  return (
    <div className="grid lg:grid-cols-3 grid-cols-1 gap-y-8 gap-x-5 text-left">
      {profiles?.map((profile, index) => (
        <div className="w-full flex flex-col items-center" key={index}>
          <div className="relative">
            <ImgLoader
              src={profile.image?.data?.attributes?.url}
              width={profile.image?.data?.attributes?.width}
              height={profile.image?.data?.attributes?.height}
