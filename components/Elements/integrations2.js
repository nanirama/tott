...
export default function Integrations2({ data }) {
  return (
    <div className="relative  py-10">
      <div className="mx-auto px-4 text-center lg:max-w-7xl">
        <div className="text-base text-center text-neutral-600">
          Integrations
        </div>
        {data?.title && (
          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {data?.integrations &&
              data?.integrations?.map((integration, index) => (
                <div key={index} className="md:pt-6 mb-5">
                  <div className="flow-root rounded-lg bg-gray-50 h-full px-6 pb-8">
                    <span className="-mt-8 inline-flex items-center justify-center rounded-md p-3">
                      <ImgLoader
                        src={integration?.image?.data.attributes.url}
                        width={51}
                        height={51}
                        alt={
                          integration?.image?.data.attributes.alternativeText
                        }
                      />
                    </span>

                    <h3 className="mt-3 text-lg text-xl tracking-tight font-medium text-gray-900">
                      {integration.title}
                    </h3>
